...
'''
    ------------------------------------------------------------------------    
                STAR DETECTION COMPLETENESS TEST - MULTICORE MODE
    ...
    Options:
      ...
      -f FILE, --filename=FILE
                            output file name
      -m MODE, --mode=MODE  modes (pool or process) [default is process]
      -n NCPUS, --ncpus=NCPUS
...
# Divide the number of iterations into smaller chunks
...
def plotter(resfile):
    ...
    try:
        ...
        try:
              ...
              print >> sys.stderr, 'Warning: Not able to close the input file ', resfile
...
def monteCarlo(image, psf, minmag, maxmag, magwidth, nstar, sid, niter, temp):
    ...
    if line_cnt > 0:
          ...
          # Determine the input and output star counts and populate the python list
    ...
    # Delete all the intermediate temp files
    if temp == 'y':
...
# Worker function for process/queue multicore processing 
def process_worker(s_q, r_q, image, psf, nstar, minmag, maxmag, magwidth, niter, temp):
    for value in iter(s_q.get, 'STOP'):
        result = monteCarlo(image, psf, minmag, maxmag, magwidth, nstar, value, niter, temp)
        r_q.put(result)


# Completeness function
def completeness(image, psf, minmag, maxmag, nstar, niter, magwidth, outfile, mode, ncpus, temp):
    # Load IRAF packages
    loadPackages()
    ...
    if mode == 'process':
        ...
        # Start addStar processes equal to number of processors
        for i in range(n_cpus):
            Process(target = process_worker, args = (send_q, recv_q, image, psf, nstar, minmag, maxmag, magwidth, niter, temp)).start()

        # Put elements in the send queue for processing
        for value in range(niter):
            send_q.put(value)
        
        # Receive results in a python list
        results = []
        ...
        # Create python list to send to processes
        #sids = getChunks( range(niter), 12 )
        idata = []
        for value in range(niter):
              idata.append((value, image, psf, minmag, maxmag, magwidth, nstar, niter, temp))  

        # Define a pool of processes and start the processing    
        pool = mp.Pool(n_cpus)

        # Receive the results in python list
        results = pool.map(pool_worker, idata)    
    ...
    # Sort the dictionary on the basis of keys and write results to output file
...
def main(image, psf, minmag, maxmag, nstar, niter, magwidth = 0.5, outfile = None, mode = 'process', ncpus = None, temp = 'y'):
    ...
    completeness(image, psf, minmag, maxmag, nstar, niter, magwidth, outfile, mode, ncpus, temp) 
...
if __name__ == '__main__':
    ...
    parser.add_option("-m", "--mode", dest = "mode", metavar="MODE",
                    action="store", help = "modes (pool or process) [default is process]",
    ...
    main(args[0], args[1], args[2], args[3], args[4], args[5], options.magwidth, options.filename, options.mode, options.ncpus, options.temp)
